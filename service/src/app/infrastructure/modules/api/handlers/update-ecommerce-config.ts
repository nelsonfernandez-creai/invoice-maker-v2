...
import UpdateEcommerceConfigUseCase from '@application/use-case/update-ecommerce-config.use-case';
...
const useCase = UpdateEcommerceConfigUseCase.create({
	metadataApiService,
	embeddingService,
	vectorStore,
	configRepository,
...
/**
 * Handler to update a ecommerce config
 */
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
	try {
		...
		return HttpResponseBuilder.success('Ecommerce config updated successfully', 200);
	} catch (error: any) {
